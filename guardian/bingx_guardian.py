"""
guardian/bingx_guardian.py
BingX Reality Guardian — LIVE profit protection for BingX positions.
Polls BingX positions every 30s, applies same Guardian rules as Bybit:
- MFE >= 0.8R  → move SL to breakeven (entry)
- MFE >= 1.0R  → move SL to entry + 0.5R (partial lock)
- MFE >= 1.5R  → move SL to entry + 1.0R (tighter lock)
- Trailing after TIGHT: SL = peak - 0.5R (forward-only)
- HOLD_HOURS > 48 → timeout alert (no SL change)

Uses BingXClient.set_trading_stop() — LIVE modifications on exchange.
Persists state to guardian/bingx_state.json (BE, partial flags).
Logs every action to bingx_alerts.jsonl.
"""
import asyncio
import json
import logging
...
for _p in ("/root", "/root/tradingos", "/root/trading_brain_v4", "/opt/ubot_bingx"):
    if _p not in sys.path:
        sys.path.insert(0, _p)

logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(name)s] %(message)s")
logger = logging.getLogger("bingx_guardian")

# Paths
GUARDIAN_STATE_PATH = Path("/root/tradingos/guardian/bingx_state.json")
PROFIT_ALERTS_PATH = Path("/root/tradingos/guardian/bingx_alerts.jsonl")
TIMEOUT_ALERTS_PATH = Path("/root/tradingos/guardian/bingx_timeout_alerts.jsonl")
...
PARTIAL_THRESHOLD = 1.0  # R multiple to move SL to entry + 0.5R
TIGHT_THRESHOLD = 1.5  # R multiple to move SL to entry + 1.0R

# ─── Trailing stop (ENABLED — same config as Bybit) ──
        ...
        with open(ENV_PATH) as f:
            for l in f:
                ...
                if l and not l.startswith("#") and "=" in l:
                    k, v = l.split("=", 1)
                    if k.strip() == "BINGX_API_KEY":
                        ak = v.strip()
                    elif k.strip() == "BINGX_API_SECRET":
                        as_ = v.strip()
...
def _get_live_positions():
    """Fetch all open BingX positions. Returns list of dicts normalized to Bybit-style keys.

    BingX position fields → normalized:
      positionAmt → size (abs)
      positionSide → side (LONG→Buy, SHORT→Sell)
      avgPrice → avgPrice
      markPrice → markPrice
      stopLoss → stopLoss (0 unless conditional orders exist)
    ...
    try:
        ...
        if not ak or not as_:
            logger.error("No BingX credentials")
            return []
        client = BingXClient(api_key=ak, api_secret=as_)
        ...
        for p in positions:
            ...
            if amt == 0:
                continue
            bside = p.get("positionSide", "LONG")
            normalized.append({
                "symbol": p.get("symbol", ""),
                "side": "Buy" if bside.upper() == "LONG" else "Sell",
                "size": abs(amt),
                ...
                "createdTime": 0,  # BingX doesn't expose createdTime in positions
            })
        return normalized
    except Exception as e:
        logger.error(f"BingX positions error: {e}")
        return []


def _get_ticker(symbol):
    """Get current price for a symbol via BingX API."""
    try:
        sys.path.insert(0, "/root/trading_brain_v4")
        from exchange.bingx.client import BingXClient
        ak, as_ = _load_credentials()
...
def _set_trading_stop(symbol, stop_loss=None, take_profit=None):
    """Set SL/TP on live BingX position via conditional orders.
    Uses BingXClient.set_trading_stop() which creates STOP_MARKET/TAKE_PROFIT_MARKET orders."""
    try:
        sys.path.insert(0, "/root/trading_brain_v4")
        from exchange.bingx.client import BingXClient
        ak, as_ = _load_credentials()
        if not ak or not as_:
            return False
        client = BingXClient(api_key=ak, api_secret=as_)
        # Determine close side from position direction
        pos_side = None
        ...
        for p in positions:
            ...
            if amt != 0:
                bside = p.get("positionSide", "LONG")
                pos_side = "Sell" if bside.upper() == "LONG" else "Buy"
                break
        if pos_side is None:
            logger.warning(f"No position for {symbol} — cannot set SL/TP")
            client.close()
            return False
        ok = client.set_trading_stop(
            symbol=symbol,
            side=pos_side,
        ...
        client.close()
        if ok:
            logger.info(f"✅ BingX SL/TP updated for {symbol}: SL={stop_loss} TP={take_profit}")
            return True
        else:
            logger.error(f"❌ Failed to update SL/TP for {symbol}")
            return False
    except Exception as e:
        logger.error(f"❌ BingX API error on {symbol}: {e}")
        return False
...
def _fetch_actual_close_price(symbol: str, side: str, fallback: float) -> float:
    """Fetch actual exit price — BingX has no closed-pnl endpoint via our client,
    so use current mark price as approximation."""
    try:
        sys.path.insert(0, "/root/trading_brain_v4")
        from exchange.bingx.client import BingXClient
        ak, as_ = _load_credentials()
