...
# ─── Trailing stop (ENABLED 2026-08-02 per user profitability request) ──
# Code kept stable since v1.4.1. Code path activates only AFTER TIGHT fires
# (peak >= +1.5R) so trailing never affects losing positions.
# Code path: peak - 0.5R (locked profit from peak), TP raised to peak + 1.0R.
TRAILING_ENABLED = True           # P0: ENABLED 2026-08-02
...
def _close_position(symbol: str, side: str, qty: float) -> bool:
    ...
    try:
        ...
        recv_window = "5000"
        close_side = "Sell" if str(side).lower().startswith("buy") else "Buy"
        ...
        r = httpx.post("https://api.bybit.com/v5/order/create",
...
def _process_position(pos, state):
    ...
    try:
        ...
        _soft_sl_recovery = False
    if _soft_sl_recovery and not sym_state.get("recovery_attempted", False):
    ...
    if not recovery_active and sym_state["mfe_peak"] >= BE_THRESHOLD and not sym_state.get("be_fired", False):
    ...
    if TRAILING_ENABLED and not recovery_active and sym_state.get("tight_fired", False):
        ...
        try:
            ...
            if d.get("retCode") == 0:
                ...
                if items:
                    it = items[0]
                    return {
                        "avgEntryPrice": float(it.get("avgEntryPrice", 0)),
                        "avgExitPrice": float(it.get("avgExitPrice", 0)),
                        "qty": float(it.get("qty", 0)),
                        "closedPnl": float(it.get("closedPnl", 0)),
                        "openFee": float(it.get("openFee", 0)),
                        "closeFee": float(it.get("closeFee", 0)),
...
def _confirm_position_closed(symbol: str, attempts: int = 2) -> bool:
    ...
    for _ in range(attempts):
        try:
            ...
            r = httpx.get(f"https://api.bybit.com/v5/position/list?{q}", headers=headers, timeout=5)
            d = r.json()
            if d.get("retCode") == 0:
...
def _record_trade_closure(symbol, state_entry):
    ...
    outcome = "UNKNOWN"
    if tp_initial and abs(close_price - tp_initial) <= tolerance:
        outcome = "TP"
    ...
    try:
        ...
        get_guard().on_trade_closed(net_pnl)
    except Exception as e:
        ...
        try:
            ...
            for sym in closed_symbols:
                if sym in state and state[sym] is not None:
                    ...
                    del state[sym]            # Process live positions
