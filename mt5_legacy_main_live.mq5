...
//--- Shadow Mode (v1.0) — SET TO FALSE FOR LIVE
...
void ExecuteShadow(Intent &intent)
{
   ...
   Print("[SHADOW] Intent #", intent_count, " | ", intent.direction, " | EQS=", DoubleToString(eqs, 2), " | Fill=", DoubleToString(fill_prob * 100, 1), "% | ", filled ? "SIMULATED" : "REJECTED");
}
...
void InitializeShadowLog()
{
   ...
   {
      FileWrite(handle, "timestamp","symbol","intent_id","direction","strength","risk_budget","spread","atr","adx","slippage","impact","fill_prob","eqs","filled");
      FileClose(handle);
   }
}

void LogShadowRecord(Intent &intent, double slippage, double impact, double fill_prob, double eqs, bool filled)
{
   ...
   if(handle != INVALID_HANDLE)
   {
      ...
      FileWrite(handle, TimeToString(TimeCurrent(), TIME_DATE|TIME_SECONDS), _Symbol, intent.id, intent.direction,
                DoubleToString(intent.confidence, 4), DoubleToString(intent.risk_budget, 4),
                DoubleToString(State.market.spread, 2), DoubleToString(State.market.atr, 4),
                DoubleToString(State.market.adx, 2), DoubleToString(slippage, 5),
                DoubleToString(impact, 5), DoubleToString(fill_prob, 4), DoubleToString(eqs, 2), filled ? "1" : "0");
      FileClose(handle);
   }
}
